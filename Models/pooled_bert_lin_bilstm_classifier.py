...
class CustomDataset(Dataset):
    def __init__(self, dataset, aug_hate, aug_neither, num_hate, num_neither ,tokenizer, class_type="all"):
        ...
        self.tweets = [tokenizer(tweet, max_length=200, padding='max_length', truncation=True, return_tensors="pt") for tweet in dataset['tweet']]
        self.labels.extend(list(aug_hate['label'])[:num_hate])
        self.tweets.extend([tokenizer(tweet, max_length=200, padding='max_length', truncation=True, return_tensors="pt") for tweet in aug_hate['tweet'][:num_hate]])
        self.labels.extend(list(aug_neither['label'])[:num_neither])
        self.tweets.extend([tokenizer(tweet, max_length=200, padding='max_length', truncation=True, return_tensors="pt") for tweet in aug_neither['tweet'][:num_neither]])
...
class BertSeqPoolLinClassifier(nn.Module):
    def __init__(self):
        ...
        self.bi_lstm = nn.LSTM(input_size=768, hidden_size=384, batch_first=True, dropout=0, bidirectional=True)
    ...
    def forward(self, input_ids, bert_mask):
        ...
        bilstm_output, _ = self.bi_lstm(seq_last_hidden_states)
        # print("Bislstm shape: ", bilstm_output.shape)
        pooled_hidden_states = self.pooling_layer(bilstm_output)
        # pooled_hidden_states = bilstm_output
...
def train(model, train_data, test_data, learning_rate_bert, learning_rate_lin, epochs, device):
    ...
    optimizer = torch.optim.AdamW([
        ...
        {'params': model.bi_lstm.parameters()},
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if curr_score > best_score:
            ...
            torch.save(model, "./bert-bilstm-fine.pth")
        # save best model
...
train(model, train_data, test_data, 0.000001, 0.0035, 5, device)
