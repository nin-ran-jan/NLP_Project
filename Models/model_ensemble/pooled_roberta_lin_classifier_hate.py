...
class BertSeqPoolLinClassifier(nn.Module):
    def __init__(self):
        ...
        self.bert = RobertaModel.from_pretrained('cardiffnlp/roberta-base-hate', force_download=True)
...
def train(model, train_data, test_data, learning_rate_bert, learning_rate_lin, epochs, device):
    model.train()
    loss_function = nn.CrossEntropyLoss(torch.tensor([1.0, 10.0]))
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if curr_score > best_score:
            ...
            torch.save(model.bert, "./weight-hate-model-bert.pth")
        # save best model
