...
def train(model, train_data, test_data, learning_rate_bert, learning_rate_lin, epochs, device):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if curr_score > best_score:
            ...
            torch.save(model, "./large-finetune-pooled-bert-2-lin-3-best.pth")
...
for lr_bert in np.linspace(1e-6, 1e-5, 10):
    for lr_lin in np.linspace(5e-4, 5e-3, 10):
        train(model, train_data, test_data, lr_bert, lr_lin, 5, device)


print("TRAIN LENGTH: ", len(train_data))

train(model, train_data, test_data, 0.00003, 0.002, 5, device)
